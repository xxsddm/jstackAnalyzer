class Visualizer {
    ...
    static createThreadElement(thread) {
        ...
        const stateClass = `state-${normalizedState ? normalizedState.replace(/_/, '-').split(' ')[0] : 'UNKNOWN'}`;
        
        threadElement.innerHTML = `
            <div class="thread-header">
                <span class="thread-name">${thread.name}</span>
                <span class="thread-state ${stateClass}">${displayState || 'UNKNOWN'}</span>
            </div>
            <div class="thread-details">${thread.details || ''}</div>
...
class JStackValidator {
    ...
    static validate(data) {
        ...
        if (!hasThreadPattern) {
            return {
                isValid: false,
                error: "未检测到有效的线程记录，请确认是否为标准jstack输出格式"
            };
        }
        
        // 检查是否包含堆栈跟踪信息
        if (!data.includes('at ') && !data.includes('java.lang.Thread.State')) {
            return {
                isValid: false,
                error: "未检测到堆栈跟踪信息，请确认是否为完整的jstack输出"
            };
        }
